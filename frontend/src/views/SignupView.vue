  ...
  <div class="min-h-screen bg-stone-50 flex items-center justify-center px-4 py-10">
    <div class="w-full max-w-sm">
      <div class="flex flex-col items-center mb-8">
        ...
        <h1 class="font-display text-2xl font-semibold text-stone-900">Create your account</h1>
        <p class="text-stone-500 text-sm mt-1">Start building a vocabulary that sticks.</p>
      </div>
      ...
      <form class="rounded-xl2 bg-white border border-stone-200 shadow-soft px-6 py-7 flex flex-col gap-4" @submit.prevent="handleSubmit">
        ...
        <div>
          ...
          <InputText id="email" v-model="email" type="email" class="w-full" autocomplete="email" placeholder="you@example.com" />
        </div>
        ...
        <div>
          <label for="confirm-password" class="block text-sm font-medium text-stone-700 mb-1.5"
          ...
          />
          <p v-if="passwordsMismatch" class="text-xs text-pink-600 mt-1">Passwords don't match.</p>
        </div>
      ...
      <p class="text-center text-sm text-stone-500 mt-5">
        ...
        <router-link to="/login" class="text-pink-600 font-medium hover:text-pink-700"
          >Log in</router-link
        >
      </p>
    </div>
