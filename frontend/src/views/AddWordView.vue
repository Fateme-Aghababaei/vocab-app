...
<template>
  <div class="max-w-3xl mx-auto flex flex-col gap-6">
    <header>
      <h1 class="text-2xl font-semibold">Add Vocabulary</h1>
      <p class="text-stone-500 mt-1">Look up a single word or extract key vocabulary from real-world text.</p>
    </header>
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center justify-between gap-2 mb-1">
                <div class="flex items-center gap-2">
                  <h3 class="font-display font-bold text-lg text-stone-900">{{ item.word }}</h3>
                  <SpeakButton :text="item.word" size="sm" />
                </div>
                <DifficultyBadge :difficulty="item.difficulty" />
              </div>

              <p class="text-sm text-stone-700 mb-2">{{ item.definition }}</p>
              ...
              <div class="flex flex-wrap gap-1">
                <CategoryChip v-for="c in item.categories" :key="c">{{ c }}</CategoryChip>
              </div>
            </div>
          </div>
        </div>
