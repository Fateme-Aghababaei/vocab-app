    ...
    <div class="[perspective:1600px]">
      ...
      >
        ...
        >
          ...
          <span class="text-xs font-medium text-faint mb-3">Do you remember this word?</span>

          <div class="flex items-center justify-center gap-3 mb-6">
            <h2 class="font-display text-4xl sm:text-5xl font-semibold text-heading">
              {{ word.word }}
            </h2>
            <SpeakButton :text="word.word" size="lg" />
          </div>

          <p v-if="word.pronunciation" class="text-lg text-quiet mb-6 break-words" title="US English IPA">
            {{ word.pronunciation }}
          </p>
          <button
            type="button"
        ...
        >
          <div class="flex items-start justify-between gap-3 mb-3">
            <div class="flex items-center gap-2.5">
              <h2 class="font-display text-2xl sm:text-3xl font-semibold text-heading">
                {{ word.word }}
              </h2>
          ...
          <p v-if="word.pronunciation" class="text-sm text-quiet mb-3 break-words" title="US English IPA">
            {{ word.pronunciation }}
          </p>
          <p class="text-copy leading-relaxed mb-4">
            {{ word.definition }}
          </p>

          <div v-if="word.examples.length" class="mb-4">
            <h3 class="text-xs font-semibold uppercase tracking-wide text-faint mb-1.5">
              Examples
            </h3>
            <ul class="space-y-2">
              <li
                ...
                :key="i"
                class="flex items-start gap-2 text-sm text-secondary italic leading-relaxed"
              >
                <SpeakButton
                  :text="ex"
                  size="sm"
          ...
          <div v-if="word.collocations.length" class="mb-4">
            ...
            <div class="flex flex-wrap gap-1.5">
              <span
                ...
                :key="i"
                class="rounded-full bg-warning-muted text-warning-ink text-xs font-medium px-2.5 py-1"
              >
                {{ c }}
              </span>
            </div>
